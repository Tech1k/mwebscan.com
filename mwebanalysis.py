...
def compute_cache(cur):
    ...
    queries = {
        ...
        'timeseries_daily': "SELECT CAST(block_time/86400 AS INT) AS day, MAX(block_height) AS height, AVG(supply) AS supply, SUM(pegin_amount) AS pegin, SUM(pegout_amount) AS pegout, SUM(pegin_count) AS pegins, SUM(pegout_count) AS pegouts FROM mweb_blocks WHERE block_time IS NOT NULL GROUP BY day ORDER BY day",
    }
    now = int(time.time())
